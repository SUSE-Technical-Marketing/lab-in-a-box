...
ism._validate(v2)
check("_validate: podman mode without smlm_scc_regcode/product reports both missing",
      any("smlm_scc_regcode" in e for e in v2.errors) and any("smlm_scc_product" in e for e in v2.errors))
...
def run_setup_smlm_podman(cfg, transactional):
    ...
    def fake_ssh_run(hostname, cmd, check=True, capture=False):
        calls.append(cmd)
    ...
    return calls, guard_calls, active_calls, sc_calls
...
calls, guard_calls, active_calls, sc_calls = run_setup_smlm_podman(cfg, transactional=True)

check("setup_smlm_podman: registers the base product via SUSEConnect -r",
      any("SUSEConnect -r REGCODE123 -e admin@lab.local" in c for c in calls))
check("setup_smlm_podman: registers the SMLM extension module via SUSEConnect -p",
      any("SUSEConnect -p SUSE-Manager-Server/5.2/x86_64" in c for c in calls))
...
calls2, _, _, _ = run_setup_smlm_podman(cfg, transactional=False)
...
_, _, _, sc_calls_keys = run_setup_smlm_podman(cfg_with_keys, transactional=True)
prefixes_used = set()
